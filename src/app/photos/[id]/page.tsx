...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const series = (PHOTO_SERIES as readonly PhotoSeries[]).find((s) => s.id === decodeURIComponent(id));
  if (!series) return {};
  return {
...
export default async function PhotoSeriesPage({ params }: Props) {
  ...
  const series = (PHOTO_SERIES as readonly PhotoSeries[]).find((s) => s.id === decoded);
  if (!series) notFound();
